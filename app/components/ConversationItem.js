...
var ConversationItem = React.createClass({
      ...
      render: function() {
        return ( < li onClick = {
            ...
            className = {
              this.props.isActiveConversation ? 'activeConv conversation-item' : 'inactiveConv conversation-item'
            } >

            {!this.props.isEditState ? < h2 className = 'conversation-item-header' > {
                this.state.conversationName
              } < /h2> : < input type = "text"
              className = "conversationname-form"
              placeholder = {
                this.state.conversationName
              }
              />}
